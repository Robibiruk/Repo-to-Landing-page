...
const FOOTER_LINKS = [
  ...
  { heading: "Community", links: [
    { label: "GitHub", href: "https://github.com/Robibiruk" },
    { label: "LinkedIn", href: "https://www.linkedin.com/in/robel-biruk-5923101b5/" },
    { label: "X (Twitter)", href: "https://x.com/ynwrobii" },
  ]},
]
...
function HomePage() {
  ...
  const doGenerate = async (url: string) => {
    setStep("analyzing"); setError(null); setResult(null)
    const timers = [
    ...
    try {
      ...
      timers.forEach(clearTimeout)
      setError(e instanceof Error ? e.message : "Something went wrong"); setStep("error")
    }
  }

  const handleTheme = async (id: string) => {
    setTheme(id); if (!result) return; setRefreshing(true)
    try {
      const res = await preview(result.content_id, id)
      setResult((prev) => prev ? { ...prev, html: res.html, theme: res.theme } : prev)
    } catch (e) { setError(e instanceof Error ? e.message : "Failed to switch theme") }
    ...
    <>
      ...
      <section className="relative flex flex-col items-center overflow-hidden px-4 pt-32 pb-20 text-center">
        <div className="pointer-events-none absolute inset-0 -z-10" aria-hidden="true">
          <Lightfall
            ...
            speed={0.4} streakCount={2} streakWidth={1.2} streakLength={1.1}
            glow={0.9} density={0.5} twinkle={0.8} zoom={3} backgroundGlow={0.4}
            opacity={1} mouseInteraction mouseStrength={0.4} mouseRadius={1.2}
            mixBlendMode="screen"
          />
          <div className="absolute bottom-0 left-0 right-0 h-40" style={{ background: "linear-gradient(to bottom, transparent 0%, #020617 100%)" }} />
        </div>
        ...
        <div className="fade-up mx-auto mt-12 w-full max-w-4xl rounded-xl border border-border/60 bg-card/80 shadow-2xl shadow-primary/5 backdrop-blur-sm">
          ...
          {result ? (
            <iframe title="Landing page preview" sandbox="" srcDoc={result.html} className="h-[60vh] w-full bg-white" />
          ) : (
            <div className="flex h-[60vh] items-center justify-center bg-gradient-to-br from-muted/20 to-muted/5">
              <p className="text-muted-foreground">Your generated landing page will appear here</p>
            </div>
          )}
        </div>
      </section>
      ...
      {result && (
        <section ref={resultRef} className="mx-auto max-w-5xl space-y-5 px-4 py-12">
          ...
          <ActionBar contentId={result.content_id} theme={result.theme} html={result.html} repoUrl={result.repo.url} repoName={result.repo.full_name} onRegenerate={() => doGenerate(result.repo.url)} disabled={refreshing} />
          <RewritePanel content={result.content} contentId={result.content_id} theme={result.theme} disabled={refreshing} onRewritten={handleRewrite} />
        </section>
      )}

      {/* ——— TRUSTED BY ——— */}
      <section className="fade-up border-y border-border/40 bg-card/50 py-10 text-center">
        <p className="mb-6 font-mono text-xs font-semibold uppercase tracking-[0.1em] text-muted-foreground">Trusted by hackathons &amp; indie hackers</p>
        <LogoLoop logos={TECH_LOGOS} speed={80} direction="left" logoHeight={32} gap={48} pauseOnHover fadeOut fadeOutColor="#020617" className="mx-auto max-w-4xl" />
      </section>

      {/* ——— FEATURES ——— */}
      <section className="fade-up px-4 py-32 text-center">
        <h2 className="text-gradient mx-auto mb-4 max-w-md text-3xl font-bold tracking-tight sm:text-4xl">AI-Powered Developer Experience</h2>
        ...
        <div className="mx-auto grid max-w-5xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f) => (
            <div key={f.title} className="glass-card fade-up group rounded-xl px-8 py-10 text-left transition hover:-translate-y-1 hover:shadow-xl">
              <div className="mb-5 flex size-12 items-center justify-center rounded-xl bg-primary/10 text-primary"><f.icon className="size-6" /></div>
    ...
    <div className="min-h-screen overflow-x-hidden">
      ...
      {isHome ? <HomePage /> : (
        <div className="min-h-screen">
          {/* Page content rendered by routes in main.tsx */}
        </div>
      )}
      ...
      <footer className="border-t border-border/40 bg-card/30 px-4 py-16">
        <div className="mx-auto grid max-w-5xl gap-10 sm:grid-cols-2 md:grid-cols-4">
          <div>
            ...
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">Turn any open-source project into a polished product page.</p>
          </div>
          {FOOTER_LINKS.map((col) => (
            <div key={col.heading}>
              <h4 className="mb-4 text-sm font-semibold uppercase tracking-wide text-foreground">{col.heading}</h4>
              <ul className="space-y-2 text-sm text-muted-foreground">
                {col.links.map((link) => (
                  <li key={link.label}>
                    {"href" in link ? (
                      <a href={link.href} target="_blank" rel="noopener noreferrer" className="transition hover:text-primary">{link.label}</a>
                    ) : (
        ...
        <div className="mx-auto mt-12 max-w-5xl border-t border-border/40 pt-8 text-xs text-muted-foreground">
          © 2026 RepoPages. Built by <a href="https://x.com/ynwrobii" target="_blank" rel="noopener noreferrer" className="hover:text-primary">Robel Biruk</a>.
        </div>
      </footer>
    </div>
  )
